...
def run_spin_echo(
    ...
    p90_us:         float
                    actual desired 90 time of the probe in us
    repetition_us:  float
    ...
    print("*****************************")
    print("DESIRED P90:",p90_us)
    print("DESIRED P180:",2*p90_us)
    prog_p90_us = prog_plen(p90_us)
    prog_p180_us = prog_plen(2*p90_us)
    print("***********************************")
    print("RUNNING AN ECHO WITH 90 TIME OF:",prog_p90_us)
    print("180 TIME IS:",prog_p180_us)
    for nScans_idx in range(nScans):
        ...
        raw_data = getData(data_length, nPoints, nEchoes, nPhaseSteps,"thisIsntFixedYet")
        run_scans_time_list.append(time.time())
