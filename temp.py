...
tau_adjust = 0.0
tau = 3500#5e3#deadtime + acq_time*1e3*(1./8.) + tau_adjust
...
for index,val in enumerate(amp_list):
    ...
    # should work same as line below and be more effic
        ...
        data.name('signal')
        if index == 0:
            nutation_data = ndshape([len(amp_list),len(time_axis)],['amp','t']).alloc(dtype=np.complex128)
            nutation_data.setaxis('amp',amp_list).set_units('amp','arb_units')
